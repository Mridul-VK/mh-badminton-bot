...
const { getBotVariable, getAdmin } = require("./utils/dbAccess.js");
...
bot.start(async (ctx) => {
  if (ctx.chat.type == "private") {
    ...
    const res = await getBotVariable("startMessage");
    ctx.reply(res.replaceAll("\\" + "n", "\n"), {
      parse_mode: "Markdown",
    });
  }
});
...
bot.help(async (ctx) => {
  ...
  for (let fileName of fileNames) {
    ...
    if (commandFile.isAdmin)
      adminCommandArray.push(`/${commandName} - ${commandDesc}`);
  ...
  ctx.reply(
    `🤝 *Help Desk* 🤝\n${"-".repeat(
      ...
      adminIds.find((id) => id == ctx.from.id)
