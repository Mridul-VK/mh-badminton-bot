...
removeAdminScene.enter(async (ctx) => {
    ...
    const { admins } = res;
    if (!admins.length) {
        await ctx.reply('Oho! it appears that there aren\'t any admins at the moment. Maybe add a few to remove them 😜');
...
removeAdminScene.on("callback_query", async (ctx) => {
    ...
    ctx.callbackQuery.message ? await ctx.deleteMessage(ctx.callbackQuery.message.message_id) : null;

    await db.query('DELETE FROM admin WHERE user_id = $1', [ctx.callbackQuery.data]);

    await ctx.reply(`Alright! Admin with user id ${ctx.callbackQuery.data} has successfully been removed`);
