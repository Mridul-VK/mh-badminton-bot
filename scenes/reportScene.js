...
const { getAdmin, getBotVariable } = require("../utils/dbAccess");
...
reportScene.on(message("text"), async (ctx) => {
  ...
  let dbAdmins = await getAdmin();
  let adminIds = dbAdmins.map((admin) => admin.user_id);
  let owner = await getBotVariable("owner");
    ...
    try {
      await ctx.telegram.sendMessage(
        ...
        `*🔔 New report received! 🔔*\n\nDate: ${new Date().toLocaleString(
          ...
          15
        )}\n\nIf you'd like to respond to the issuer, click on the name of the issuer`,
        { parse_mode: "Markdown" }
      );
    } catch (error) {
      if (error.response && error.response.error_code == 403)
        console.log(
          `${dbAdmins.rows.find(
            (admn) => admn.user_id == admin
          )} has never interacted with the bot`
        );
    }
  });
