...
module.exports = resetSlots = async () => {
  try {
    ...
    const currentDatetime = new Date(
      utcTime + 5 * 60 * 60 * 1000 + 31 * 60 * 1000
    );
    await updateBotVariable("currentDatetime", currentDatetime.getTime());
    await clearBooking();
    ...
    for (let i = 0; i < 7; i++) {
      await updateBooking(currentDatetime.getTime() + i * 60 * 60 * 1000);
    }
  } catch (error) {
